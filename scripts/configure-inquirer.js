...
if (!process.stdin.isTTY) {
  console.error(chalk.red('This tool requires an interactive terminal. Please run it directly in your terminal.'));
  process.exit(1);
}
...
async function runCLI() {
  ...
  console.log(chalk.gray(`Current theme: ${chalk.white(currentConfig.primaryColor)} ${getColorPreview(currentConfig.primaryColor)}`));
  console.log();
  ...
  try {
    ...
    console.log(`Primary color will be set to: ${chalk.white.bold(selectedColor)} ${getColorPreview(selectedColor)}`);
    console.log();
    ...
    if (writeConfig(newConfig)) {
      ...
      console.log(chalk.gray(`Primary color set to: ${selectedColor} ${getColorPreview(selectedColor)}`));
      console.log();
